...
export default function ReviewActions({ review }: { review: any }) {
  ...
  return (
    <div className="flex space-x-2 mt-2">
      <button onClick={deleteReview} className="text-red-600">Hapus</button>
      <button onClick={flagReview} className="text-yellow-600">Flag</button>
      <button onClick={cancelFlag} className="text-green-600">Cancel Flag</button>
    </div>
  )
}
